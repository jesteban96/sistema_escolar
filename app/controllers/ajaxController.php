...
class ajaxController extends Controller {
  ...
  function getMateriasDisponiblesProfesor(){
    try {
      ...
      $materias = materiaModel::disponiblesProfesor($profesor['id']);
  ...
  function getMateriasProfesor(){
    try {
      ...
      $materias = materiaModel::materiasProfesor($profesor['id']);
  ...
  function addMateriaProfesor()
  {
    try {
      ...
      if(materiaModel::list('materiasxprofesores',['id_materias'=>$id_materia,'id_profesores'=>$id_profesor])){
        throw new Exception(sprintf('La materia <b>%s</b> ya está asignada al profesor <b>%s</b>.',$materia['nombre'],$profesor['nombre']));
      }

      //Asignar la materia al profesor
      if(profesorModel::asignarMateria($id_profesor,$id_materia)===false){
        throw new Exception(getNotification(2));
      }

      $msg = sprintf('Nueva materia <b>%s</b> asignada con exito.',$materia['nombre']);
