$(document).ready(function() {
  ...
  function getMateriasDisponiblesProfesor() { 
    ...
    $.ajax({
      url:'ajax/getMateriasDisponiblesProfesor',
      type:'get',
      dataType:'json',
      data:{
        '_t':Bee.csrf,
    ...
    .fail(function(err){
      toastr.error('Error en la Petición. Upps!!!');
    })
    .always(function(){
      wrapper.waitMe('hide');
    })
  }
  getMateriasDisponiblesProfesor();
  ...
  $('#asignarMateriaForm').on('submit',addMateriaProfesor);
  function addMateriaProfesor(e) {
    ...
    if(id_materia === undefined || id_materia === ''){
      toastr.error('Seleccione una materia válida');
      return
    }

    // //AJAX
    $.ajax({
      url:'ajax/addMateriaProfesor',
      type:'post',
      dataType:'json',
      data:{
        csrf,
